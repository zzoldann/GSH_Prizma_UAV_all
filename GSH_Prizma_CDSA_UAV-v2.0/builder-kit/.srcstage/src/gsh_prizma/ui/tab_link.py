...
# -- injected:xaxis-radiogroup --
        try:
            self.xaxis_group_box = QtWidgets.QGroupBox(load_help_dict().get('x_axis_mode',{}).get('label','Шкала оси X'))
            ...
            apply_help_key(self.xaxis_group_box, 'x_axis_mode')
...
def apply_help_key(w,key,default=None):
    h=load_help_dict().get(key,{})
