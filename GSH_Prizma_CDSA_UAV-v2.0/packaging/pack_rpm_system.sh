...
if ! command -v fpm >/dev/null 2>&1; then
  echo "[!] fpm not found. Try: sudo gem install --no-document fpm"
  exit 1
fi
...
fpm -s python -t rpm "$WHEEL"     --name "$NAME"     --version "$VER"     --python-bin python3     -d "python3 >= 3.9"     -d "python3-matplotlib"

echo "[i] Done. See *.rpm in $(pwd)"
